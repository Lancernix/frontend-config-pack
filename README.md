...
├── packages/configs/        # 要发布的 npm 包 @bubu/configs
│   ├── oxlint/              # 三套框架预设，分开维护
...
1. 拷贝 `packages/configs/oxlint/`、`packages/configs/tsconfig/`、`packages/configs/templates/` 内容及 `.husky/`、`.vscode/` 到项目根（或发布后直接装 `@bubu/configs`，见 `packages/configs/README.md`）。
2. 装依赖：
